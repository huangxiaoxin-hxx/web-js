<template>
  <div class="categoryList">
    <scroll-view scroll-x class="head" :scroll-left="scrollLeft">
      <div v-for="(item, index) in navData" :key="index" :class="[nowIndex == index ? 'active':'']" @click="changeTab(index, item.id)">{{item.name}}</div>
    </scroll-view>
    <div class="info">
      <p>{{currentNav.name}}</p>
    ...
    <div class="list" v-if="goodsList.length !== 0">
      <div class="item" @click="goodsDetail(item.id)" v-for="(item, index) in goodsList" :key="index">
        <img :src="item.list_pic_url" alt="">
        <p class="name">{{item.name}}</p>
        <p class="price">{{item.retail_price}}</p>
      </div>
    </div>
    <div class="none" v-else>暂无此类商品...</div>
  </div>
</template>
